...
let bookDescription;
try {
  bookDescription = document.querySelector("#link-report").innerText;
...
// 获取热门短评的“渲染”文本
let hotShortComments;
try {
  hotShortComments = document.querySelector("#comment-list-wrapper").innerText;
  // 替换所有双引号（"）为单引号（'）
  hotShortComments = hotShortComments.replace(/"/g, "'");
} catch (error) {
  hotShortComments = "N/A";
}
